...
pub fn parse_output(input: &Input, f: &str) -> Result<Output, String> {
    ...
    for _ in 0..M {
        ...
        for i in 0..4 {
            rect[i] = (
                read(tokens.next(), 0, input.N as i32 - 1)?,
                read(tokens.next(), 0, input.N as i32 - 1)?,
            );
        }
        out.push(rect);
...
pub fn vis(input: &Input, out: &[[P; 4]], show_grid: bool, mode: i32) -> (i64, String, String) {
                    ...
                    doc = doc.add(
                        Circle::new()
                            ...
                            .set(
                                ...
                                format!(
                                    "rgb({0},{0},{0})",
                                    256 - (256 * weight((x as i32, y as i32), input.N)
                                        / weight((0, 0), input.N))
                                ),
                            ),
                    );
    ...
    for (idx, rect) in out.iter().enumerate() {
        ...
        let d2y = rect[3].1 - rect[0].1;
        let len = std::cmp::max(d1x.abs(), d1y.abs()) + std::cmp::max(d2x.abs(), d2y.abs());
        for i in 0..4 {
            let (x1, y1) = rect[i];
